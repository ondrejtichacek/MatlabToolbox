...
% 
%   POS = SUBFIGRID(NROWS,NCOLS) creates an array of positions for
%   positioning axes as subfigures. The array has dimensions [M,P,N]: M is
%   the subplot row, N is the subplot column, and P is the position vector.
%   By default, each axis will be scaled such that [width height] will be
%   [1/NCOLS 1/NROWS].
%   
%   POS = SUBFIGRID(NROWS,NCOLS,OFFSET) allows a margin OFFSET to be
%   specified. This should be a four-element vector specifying the margins
%   thus: [left right top bottom]. By default offset=[0 0 0 0]. Axes will
%   be scaled to fill the remaining space.
% 
%   POS = SUBFIGRID(NROWS,NCOLS,OFFSET,SCALE) allows the axes to be scaled.
%   This should be a two-element vector specifying a scale factor that will
%   be applied to each axis; SCALE(1) scales the width, SCALE(2) scales the
%   height. The axes will be scaled such that the offset margin will be
%   retained. By default SCALE=[1 1].
% 
