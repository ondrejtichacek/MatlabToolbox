...
% 
%   S = LTAS(X,FS) calculates the long-term average spectrum (LTAS) of
%   signal X, sampled at FS Hz. The spectrum is calculated from the average
%   power spectral density (PSD) obtained from a series of overlapping
%   FFTs; the FFT length is 4096, and the hop size is 2048. The segments of
%   X are Hann-windowed. The average PSD is then Gaussian-smoothed to
...
%   
%   S = LTAS(X,FS,'PARAMETER','VALUE') allows numerous parameters to be
%   specified. These parameters are:-
...
% =========================================================================
    ...
    if nargin>2 % if parameters are specified
        ...
        if round(nArgs/2)~=nArgs/2
           error('SPECTRUM needs propertyName/propertyValue pairs')
        end
        % overwrite defults
    ...
    for c = 1:size(x,2)
        [S,f] = stft(x(:,c),win,hop,fs); % short-time ft
        s(:,c) = mean(abs(S/NFFT).^2,2); % mean PSD
        s(:,c) = units(s(:,c)); % put into dB
        s(:,c) = smooth_spectrum(s(:,c),f,Noct); % smooth
    end
