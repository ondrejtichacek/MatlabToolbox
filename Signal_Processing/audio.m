...
% 
%   The AUDIO superclass is an abstract class that defines audio-related
%   properties and methods. As an abstract class, it cannot be
%   instantiated.
% 
%   AUDIO is a subclass of MATLAB.MIXIN.COPYABLE, which is a subclass of
%   the HANDLE class. Hence subclasses of AUDIO are handle classes.
% 
%   AUDIO properties:
...
%   Copyright 2016 University of Surrey.
    ...
    methods % public methods
        ...
        %SOUND Replay the audio signal
        %
        %   AUDIO.SOUND() replays the audio signal.
    ...
    methods (Access = protected)
        ...
        % 
        %   AUDIO.SET_PROPERTIES(VGIN,PROPNAMES) searches through the cell
        %   array VGIN (a cell array of parameter/value pairs) for
    ...
    methods (Static)
        ...
        %NORMALIZE Normalize an audio signal
        %
        %   Y = AUDIO.NORMALIZE(X) normalises X such that ABS(Y(:))<1.
        % 
        %   [Y,GAIN] = AUDIO.NORMALIZE(X) returns the scalar gain GAIN
        %   applied to X in order to normalize it.
        ...
        %SPAT Spatialise a sound using a SOFA file
        % 
        %   AUDIO.SPAT(SOFAFILE,X,AZIMUTH,ELEVATION) spatialises the
        %   signal X using spatialisation data contained in the SOFA file
        %   SOFAFILE at the corresponding AZIMUTH and ELEVATION. The
        %   sampling frequency is that of the SOFA data. X should be a
        %   column vector; if it is not, it will be down-mixed to a column
        %   vector.
        %   
        %   AUDIO.SPAT(...,FS) uses the sampling frequency FS for the
        %   spatialisation. If the SOFA data are not at the sampling rate
        %   FS, the data are resampled.
        ...
        %UP_DOWN_MIX Remix a source to a designated channel count
        % 
        %   Y = AUDIO.UP_DOWN_MIX(X,N) mixes the signal X to have the
        %   specified channel count N. Data for each channel in X should be
        %   stored down its columns. For input channel count M, the
        %   up-/down-mix coefficients are calculated as ONES(M,N)./(M*N).
