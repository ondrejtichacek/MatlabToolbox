function update_contents(folder)
%UPDATE_CONTENTS Create a Contents.m file including subdirectories
% 
%   UPDATE_CONTENTS scans through the current directory, and its
%   subdirectories, and builds a Contents file similar to Matlab's
%   report-generated Contents.m files. Any existing Contents.m file will be
%   overwritten.
%   
%   UPDATE_CONTENTS(FOLDER) scans through the directory FOLDER.
% 
%   Typing
%   
%       help(FOLDER)
...
% =========================================================================
    ...
    % get subfolders
    dirs = get_contents(folder,'filter','folders','rec',true,'path','full','sort',true);
    dirs = [{folder}; dirs];
    ...
    H1_lines = cell(0,1);
    for d = 1:length(dirs)
        temp = get_contents(dirs{d},'filter','files','sort',true);
        if ~isempty(temp)
    ...
    fprintf(fid, '%%    \n%%   %s on %s at %s.\n', 'This file was generated by update_contents.m',datestr(datetime('now'),'dd mmm yyyy'),datestr(datetime('now'),'HH:MM:SS'));
    fclose(fid);
