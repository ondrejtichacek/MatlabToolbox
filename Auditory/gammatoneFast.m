...
%
%   BM = GAMMATONEFAST(X,CFS,FS) passes the vector X through a bank of
%   fourth-order gammatone filters, with centre frequencies specified by
%   CFS. The function returns a matrix, with each row/column corresponding
%   to a filter output with a centre frequency determined by the
%   corresponding element in CFS. The orientation of the output is
...
% 
%   BM = GAMMATONEFAST(...,ALIGN) allows phase alignment to be applied.
%   With ALIGN=false, no alignment is applied (default). With ALIGN=true,
%   fine structure and envelope alignment is applied so that the impulse
%   response peaks occurs at t=0.
% 
%   [BM,ENV] = GAMMATONEFAST(...) returns the instantaneous envelopes ENV
%   for each filter.
% 
%   [BM,ENV,DELAY] = GAMMATONEFAST(...) returns the delay DELAY (in
%   samples) removed by the phase alignment of each gammatone filter, i.e.
%   DELAY(n)=0 if ALIGN=true. DELAY is a vector the same size as CFS.
%
