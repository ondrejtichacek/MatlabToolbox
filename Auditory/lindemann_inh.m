function [L_l,R_l] = lindemann_inh(L,R,fs,c_inh,dim)
%LINDEMANN_INH Signal pre-processing for Lindemann's cross-correlation
% 
%   [L_L,R_L] = LINDEMANN_INH(L,R,FS) pre-processes left L and right R
%   signals for the cross-correlation function based on Lindemann's
...
% 
%   [L_L,R_L] = LINDEMANN_INH(L,R,FS,C_INH) uses the specified inhibition
%   parameter C_INH. The value must be in the interval [0,1].
% 
%   [L_L,R_L] = LINDEMANN_INH(L,R,FS,C_INH,DIM) pre-processes L and R along
%   the dimension DIM.
% 
%   References
%   
