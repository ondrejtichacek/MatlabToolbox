...
% 
% BinSearch: Conducts a binary search and continues until
% the score is close enough to the target. An upper limit
% should be set on the number of iterations
%
%     [SearchPoint, Step, ReLoop] = BinSearch(Score, ...
%       TargetScore, ...
%       CloseEnough, ...
%       NextSearchPointa, ...
%       Step, ...
...
% - CloseEnough: the distance from the Target at which it
% is acceptable to discontinue the binary search
% - SearchPoint: the next work input value to try
% - Step: the distance which the SearchPoint can move by
% - LoopCount: a counter for the number of iterations
% completed so far
% - MaxLoops: The upper limit on the number of iterations
% allowed
%
% outputs:
% - SearchPoint: the next work input value you should try
% - Step: the distance which can be stepped on the NEXT
% iteration. You should store this value.
% - ReLoop: If this flag is set to 0, the upper level while
% loop will be terminated
%
%
% example: you wish to use an audibility model to find the
% level at which you can be 50% confident of detecting the
% signal (-+ 1%). You choose to start with an input level
% of 20dB and take no more than 10 steps of 40,20,10 dB etc.
%
...
%
%   [SignalLevel Step ReLoop] = BinSearch(Percent, ...
%       TargetPercent, ...
%       CloseEnough, ...
%       SignalLevel, ...
%       Step, ...
