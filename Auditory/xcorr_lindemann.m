function [c,lags] = xcorr_lindemann(L,R,fs,maxlag,dim)
%XCORR_LINDEMANN Cross-correlation based on Lindemann's precedence model
% 
%   C = XCORR_LINDEMANN(L,R,FS) calculates the cross-correlation of vectors
%   L and R, sampled at FS Hz, using a maximum cross-correlation lag of
%   0.001*fs samples (1 ms). The cross-correlation function is based on
%   Lindemann's [1,2] precedence model. C is the same size as L or R,
%   except that size(C,1) = 2*0.001*fs+1.
% 
%   C = XCORR_LINDEMANN(L,R,FS,MAXLAG) calculates the cross-correlation
%   using a maximum cross-correlation lag of MAXLAG samples.
% 
%   C = XCORR_LINDEMANN(L,R,FS,MAXLAG,DIM) performs the cross-correlation
%   along the dimension DIM. C is the same size as L or R, except that
%   size(C,DIM) = 2*MAXLAG+1.
% 
%   [C,LAGS] = XCORR_LINDEMANN(...) returns the lags, in seconds, over
%   which the cross-correlation was calculated.
% 
%   References
%   
...
% =========================================================================
    ...
    check_mex_compiled('-largeArrayDims','xcorr_lindemann_c.c')
    ...
    for n = 1:size(L,2)
        c(:,n) = xcorr_lindemann_c(L(:,n),R(:,n),fs,maxlag,t_inh,t_int);
    end
