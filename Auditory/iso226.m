...
%   
%   [SPL,F] = ISO226(PHON) returns the sound pressure level (SPL) (dB) of
%   pure tone frequencies F (Hz) at the loudness level(s) PHON. The values
%   are calculated according to ISO 226:2003 using the reference
%   frequencies specified in the standard. According to the standard, PHON
%   is only valid at all frequencies if 20<=PHON<80 (although the function
%   will return SPL values outside of this range).
% 
%   PHON may be an array of any size; SPL and F will be of size
%   [1,29,M,N,P,...] where M,N,P,... are the dimensions of PHON.
%
%   [SPL,F] = ISO226(PHON,FQ) returns the SPL of the pure tone frequencies
%   in FQ at the specified loudness level(s). According to the standard, FQ
%   is only valid below 12.5 kHz (although the function will extrapolate
...
%   
%   ... = iso226(phon,fq,sq) specifies whether singleton dimensions will be
%   removed from the output. With sq=false, singleton dimensions will be
%   retained (default), else they will be removed.
% 
%   ... = iso226(phon,[],sq) uses the standard reference frequencies for
%   SPL calculations.
% 
%   Example
% 
