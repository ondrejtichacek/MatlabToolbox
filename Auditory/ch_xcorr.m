...
% 
%   CCG = CH_XCORR(HC_L,HC_R,FS) cross-correlates the input 2-D matrices
...
%   dimensions [lag,frequency,frame]. The function calculates running
%   cross-correlations for every sample and integrates these
%   cross-correlations over each frame. The number of frames frame_count is
%   calculated thus:
% 
%       frame_count = ...
...
%   
%   CCG = CH_XCORR(HC_L,HC_R,FS,'PARAMETER',VALUE) allows a number of
...
% 
%   [CCG,IC] = CH_XCORR(...) returns the calculated IC to the matrix IC.
%   Although the matrix returned is the same size as hc_L, IC is only
%   calculated for samples 1:frame_count*frame_length, other values will be
%   set to 0.
% 
%   Algorithm
% 
