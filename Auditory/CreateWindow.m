...
%
% OutWin = CreateWindow(WinLen, Type, Duration)
%
% Other (non-symmetric) uses include:
%
% OutWin = CreateWindow(WinLen, ...
%   Hann, ...
%   OnDuration, ...
%   Hann, ...
%   OffDuration,)
...
%   OffSlope)
%
% OutWin = CreateWindow(type, samples)
% OutWin = CreateWindow(Ontype, Onsamples, Offtype, Offsamples)
%
% In the first mode of operation, the onset and offset are
% assumed to be identical (symmetric window).
% In other modes, the OnRamp and OffRamp durations (and
% slopes if necessary) are specified independently
%
% OutWin: the created window
%
% WinLen: the total duration (in samples) of the desired
%   window
%
% Type (inc. Ontype and Offtype):
%   
%   - 'Hann': a raised cosine ramp, following value should
% always be the Duration of the ramp
%   
%   - 'Exp': an exponential ramp, following value should be
%   always be the Duration of the ramp, and then the Slope.
%   e.g Slope > 3 very shallow, slope < 0.1 very steep
%
% Duration: number of samples over which the OnRamp and
% OffRamp are applied
%
% Slope: the steepness of the exponential ramp
% 
%
% Example case: 100 samples onset Hann, followed by 50
% unramped samples (i.e. value = 1), followed by 500 samples
% of shallow exponential offset ramp
%
