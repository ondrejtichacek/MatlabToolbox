function [ccg,ic] = ch_xcorr_2(hc_L,hc_R,fs,varargin)
%CH_XCORR_2 Calculate cross-correlograms with a range of options.
% 
%   CCG = CH_XCORR_2(HC_L,HC_R,FS) cross-correlates the input 2-D matrices
%   HC_L and HC_R over 10ms frame with a maximum lag of 1ms. It is assumed
%   that the number of frequency channels is min(size(HC_L)) and hence HC_L
%   and HC_R can be in either orientation. The cross-correlograms consist
%   of cross-correlations for every frame and frequency channel. CCG has
%   dimensions [lag,frequency,frame]. The function calculates the
%   traditional cross-correlation in each frame. The number of frames
%   FRAME_COUNT is calculated thus:
% 
%       FRAME_COUNT = FIX((MAX(SIZE(HC_L)))/FRAME_LENGTH);
%   
%   CCG = CH_XCORR_2(HC_L,HC_R,FS,'PARAMETER',VALUE) allows a number of
...
% =========================================================================
    ...
    check_mex_compiled('-largeArrayDims','ch_xcorr_c_2.c')
    ...
    if nargin > 3 % if parameters are specified
        ...
        if round(nArgs/2)~=nArgs/2
           error('CH_XCORR_2 needs propertyName/propertyValue pairs')
        end
        % overwrite defults
    ...
    [ccg,ic] = ch_xcorr_c_2(hc_L,hc_R,frame_count,frame_length,maxlag,hop,norm_flag);
