...
%   'means' for the mean markers, and 'txtsample' for text objects (if the
%   'sampleSize' option is set). The field 'txtg', for group text labels if
%   'style' is set to 'hierarchy', is also returned, but is a vector of
%   handles: one for each label.
% 
...
%   'mean'              : {false} | true
%       Display the mean of the data for each box.
%   'meanColor'         : ColorSpec | {'auto'}
%       Color of the mean marker.
%   'meanMarker'        : {'+'} | 'o' | '*' | '.' | 'x' |
%                         'square' or 's' | 'diamond' or 'd' | '^' | 'v' |
%                         '>' | '<' | 'pentagram' or 'p' | 'hexagram' or
%                         'h' | 'none'
%       Marker used for the mean (if 'mean' is true).
%   'meanSize'          : Scalar {6}
%       Size, in point units, of the mean markers (if 'mean' is true).
...
%   'lineStyle', 'lineWidth', 'meanSize', 'medianColor', 'notchLineColor',
...
% =========================================================================
% Last changed:     $Date: 2015-12-07 10:55:15 +0000 (Mon, 07 Dec 2015) $
% Last committed:   $Revision: 451 $
...
% =========================================================================
    ...
    options.meanColor = checkAutoColor(options.meanColor,groupSize);
    ...
    meanColor = groupOption(options.meanColor,groupSize,'meanColor');
    meanMarker = groupOption(options.meanMarker,groupSize,'meanMarker');
    meanSize = groupOption(options.meanSize,groupSize,'scatterSize');
    ...
    Z.mean = zeros(size(Z.median)); % sample mean
    ...
    for n = 1:prod(outdims)
        ...
        temp = y(subidxAll{:});
        Z.mean(subidx{:}) = mean(temp(~isnan(temp)));
    ...
    for n = 1:prod(outdims)
        ...
        if options.mean
            h.means(subidx{:}) = plot(x(subidx{2}) + gOffset,mean(Z.mean(subidx{:})),...
                'color',meanColor{gidx{:}},...
                'marker',meanMarker{gidx{:}},...
                'markersize',meanSize{gidx{:}});
        end
    end
