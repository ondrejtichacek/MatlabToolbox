...
%   
%   RMSE = GETRMSE(X, Y) calculates the RMSE of the inputs data where
%
...
%   have equal size.
%
%   RMSE = GETRMSE(X, Y, D) allows the degrees of freedom D to be
%   specified. The default is D=1 (you may wish to set it to 0, or the
%   degrees of freedom).
%
%   RMSEEPS = GETRMSE(X, Y, D, EPSILON) calculates epsilon-insensitive RMSE
%   (RMSE*). The parameter EPSILON is a threshold for err values: err
%   values less than the respective EPSILON value are set to 0; err values
%   greater than EPSILON have EPSILON subtracted from them. This allows for
%   the calculation of error 'after' a certain effect (such as subjective
%   error). RMSEEPS will ALWAYS be lower than RMSE. The value of EPSILON
%   will generally be set to half the 95% confidence interval or similar
